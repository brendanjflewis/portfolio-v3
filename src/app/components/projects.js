        ...
        <section id="projects" className="lg:max-w-desktop md:max-w-tablet sm:max-w-mobile mx-auto flex flex-col justify-center lg:py-[100px] md:py-[60px] sm:py-[50px] sm:px-[10px]">
            ...
            <div className="flex flex-col gap-5">
                {projects.slice(0, 3).map((project, index) => (
                    <a key={index} href={project.url} target="_blank" className='group md:flex sm:flex-col-reverse rounded-md border border-[#1b1b1b] hover:border hover:border-zinc-500 hover:backdrop-opacity-50 hover:backdrop-brightness-100 hover:bg-[#292929] ease-in-out duration-200'>
                        <div className="flex md:flex-row sm:flex-col-reverse justify-between py-3 px-2">

                            <div className="flex flex-col md:w-[50%]">
                                {/* <p className="text-sm text-[#64ffda]">{project.projectType}</p> */}
                                <div className="flex gap-2.5 items-center">
                                    <h3 className="text-slate-100 group-hover:text-[#87D0E0] ease-in-out duration-200" >{project.title}</h3>
                                ...
                                <div className="flex flex-wrap gap-2.5 justify-self-end pt-[10px]">
                                    {project.tags.map((tag, i) => (
                                        <ul key={i} className="text-sm text-[#87D0E0] backdrop-brightness-100 group-hover:backdrop-brightness-125 px-[8px] py-[1px] rounded-xl ease-in-out duration-200">
                                            <li>{tag}</li>
                                        </ul>
                                    ))
                                    }
                            ...
                            <div className="md:pr-[20px]">
                                <Image src={project.image} alt={project.imageAlt} className="md:min-h-[200px] sm:min-h-[120px] md:max-h-[200px] sm:max-h-[120px] md:min-w-[360px] sm:min-w-[240px] md:max-w-[360px] sm:max-w-[240px] object-cover rounded-[5px] border-2 border-[#262626] group-hover:border-2 group-hover:border-[#333333]" />
                            </div>
                        </div>
                    </a>
                ))
            ...
            <div className="group flex justify-end pt-[30px] md:pr-[30px]">
                <Link href="https://github.com/brendanjflewis" target="_blank" className="flex items-center gap-2.5">
                    <h3 className="text-slate-400 group-hover:text-[#87D0E0] ease-in-out duration-200">View more projects from my GitHub</h3>
